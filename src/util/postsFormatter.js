const getJsonFiles = import.meta.glob('../data/posts/*.json', { eager: true })

function replaceSwedishCharacters(string) {
  return string.replaceAll(/[ä-å]/g, 'a').replaceAll('ö', 'o')
}

const createStructuredPosts = Object.entries(getJsonFiles).map((entry) => {
  const file = entry[0]
...
const groupedPostsByMonth = createStructuredPosts.reduce((Map, current) => {
...
export { createStructuredPosts as blogPosts, groupedPostsByMonth, replaceSwedishCharacters }
